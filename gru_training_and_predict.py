...
def random_search_gru(features, target, gru_param_dist):
    ...
    #print(best_params)
    # Random Search + Cross Validation (k = 5)(GRU):
    # {'optimizer': 'rmsprop', 'epochs': 150, 'batch_size': 32} con 50 allenamenti (31 minuti circa)

def grid_search_gru(features, target, gru_param_dist):
    # definizione dello spazio di ricerca per la grid search
    grid_search = lib.GridSearchCV(
        estimator= lib.KerasRegressor(build_fn=utils.create_gru_model, input_shape=(features.shape[1], 1), verbose=0),
        param_grid=gru_param_dist,
        cv=lib.TimeSeriesSplit(n_splits=5).split(features),
        verbose=2
    )
    ...
    #print("Migliore combinazione di iperparametri secondo Grid Search (GRU):")
    #print(best_params)
    # Grid Search + Cross Validation (k = 5)(GRU):
    # {'batch_size': 32, 'epochs': 150, 'optimizer': 'adam'} con 90 allenamenti (50 minuti circa)

def fit_and_predict_gru_with_parametres_by_random_search(features, target, best_params_random_search, df):
    # creazione del modello LSTM con la migliore combinazione di iperparametri da random search
    best_model_random_search = utils.create_gru_model(input_shape=(features.shape[1], 1), optimizer=best_params_random_search['optimizer'])
...
def random_search_gru_validation(target, predictions_random_search):
...
def fit_and_predict_gru_with_parametres_by_grid_search(features, target, best_params_grid_search, df):
    # creazione del modello LSTM con la migliore combinazione di iperparametri da grid search
    best_model_grid_search = utils.create_gru_model(input_shape=(features.shape[1], 1), optimizer=best_params_grid_search['optimizer'])
...
def grid_search_gru_validation(target, predictions_grid_search):
...
def plot_gru_predictions(predicted_precipitation_random_search, predicted_precipitation_grid_search, mse_random_search, rmse_random_search, mae_random_search, mse_grid_search, rmse_grid_search, mae_grid_search):
...
def main():
    ...
    # Random Search + Cross Validation (k = 5)(GRU):
    # random_search_gru(features, target, gru_param_dist)

    # Grid Search + Cross Validation (k = 5)(GRU):
    # grid_search_gru(features, target, gru_param_dist)
    ...
    # Previsioni con modello GRU con parametri ottenuti da random search
    predictions_random_search, predicted_precipitation_random_search = fit_and_predict_gru_with_parametres_by_random_search(features, target, best_params_random_search, df)
    
    # Valutazione delle previsioni con modello GRU con parametri ottenuti da random search
    mse_random_search, rmse_random_search, mae_random_search = random_search_gru_validation(target, predictions_random_search)

    # Previsioni con modello GRU con parametri ottenuti da grid search
    predictions_grid_search, predicted_precipitation_grid_search = fit_and_predict_gru_with_parametres_by_grid_search(features, target, best_params_grid_search, df)

    # Valutazione delle previsioni con modello GRU con parametri ottenuti da grid search
    mse_grid_search, rmse_grid_search, mae_grid_search = grid_search_gru_validation(target, predictions_grid_search)

    # Grafico delle previsioni e delle metriche di valutazione
    plot_gru_predictions(predicted_precipitation_random_search, predicted_precipitation_grid_search, mse_random_search, rmse_random_search, mae_random_search, mse_grid_search, rmse_grid_search, mae_grid_search)
